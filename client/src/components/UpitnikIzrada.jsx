...
function UpitnikIzrada({ xmlData, obrnutoKodirani, min, max }) {
  const [raspon, setRaspon] = useState([]); // raspon bodova od min do max po kategorijama odgovora
  const sections = [].concat(xmlData.questionnaire.section);
  ...
  return (
    <div className="container my-3">
      {sections.map((sec, i) => {
        ...
        const cats = q.response.fixed
          ? [].concat(q.response.fixed.category)
          : [];
